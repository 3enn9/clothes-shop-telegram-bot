...
async def start(event: types.Message | types.CallbackQuery, session: AsyncSession, bot: Bot):
    if isinstance(event, types.Message):
        ...
        if hasattr(event.message, 'edit_text'):
            await event.message.edit_text(text="Добро пожаловать в наш магазин",
                                          reply_markup=inlinekeyboard_menu.inkb_start_menu)
        else:

            await bot.send_message(chat_id=event.from_user.id, text="Добро пожаловать в наш магазин",
                                   reply_markup=inlinekeyboard_menu.inkb_start_menu)
